package com.hidevelop.weatherdiary.controller;

import com.hidevelop.weatherdiary.domain.Diary;
import com.hidevelop.weatherdiary.service.DiaryService;
...
public class DiaryController {
    ...
    @ApiOperation(value = "선택한 날짜의 모든 일기 데이터를 가져옵니다", notes = "이것은 노트")
    @GetMapping("/read/diary")
    List<Diary> readDiary(@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date){
       return diaryService.readDiary(date);
    }

    @ApiOperation(value = "선택한 기간중에 모든 일기 데이터를 가져옵니다.", notes = "이것은 노트")
